...
def RMSE(predictions, targets):
    errors = np.sum(np.square(predictions - targets))
...
class OpticalNetwork:
    def __init__(self, dimensions, inputDimension=4, MeshType = MZImesh):
        ...
        for index in range(self.numLayers):
            if index == 0:
                largerDim = dimensions[index] if dimensions[index] > inputDimension else inputDimension
            else:
                largerDim = dimensions[index] if dimensions[index] > dimensions[index-1] else dimensions[index-1]
            self.layers.append({"index": index,
                                "dimension": dimensions[index],
                                "rateCode": Sigmoid,
                                "mAct": np.zeros(dimensions[index]),
                                "pAct": np.zeros(dimensions[index]),
                                "mesh": MZImesh(largerDim)
                                })
            # print(self.layers[-1])
    ...
    def Predict(self, sample, numTimeSteps = 50, deltaTime = 0.1):
        ...
        for timeStep in range(numTimeSteps):
            ...
            for index, layer in enumerate(self.layers):
                ...
                layerDimension = layer["dimension"]
                inputDimension = len(input)
                dimensionDifference = abs(layerDimension-inputDimension)
                if inputDimension < layerDimension: # zero pad if input is small
                    input = np.pad(input, (0,dimensionDifference))
                input = Diagonalize(input)
                detectorIn = layer["mesh"].getMatrix() @ input
                if index != self.numLayers - 1:
                    nextLayer = self.layers[index+1]
                    if nextLayer["dimension"] < layerDimension: # zero pad if feedback is small
                        # print(f"layer: {layer}")
                        # print(nextLayer["dimension"])
                        # print(layer["dimension"])
                        # print(nextLayer["mesh"].getFeedback().shape)
                        detectorIn += np.pad(nextLayer["mesh"].getFeedback() @ Diagonalize(np.pad(nextLayer["mAct"],
                                            (0, layerDimension-nextLayer["dimension"]))), (0, dimensionDifference))
                    else:
                        detectorIn += nextLayer["mesh"].getFeedback() @ Diagonalize(nextLayer["mAct"])
                detectorOut = Detect(detectorIn)
                layer["mAct"] += deltaTime*(layer["rateCode"](detectorOut)[:layerDimension]-layer["mAct"])
    ...
    def Observe(self, sample, target, numTimeSteps = 50, deltaTime = 0.1):
        ...
        for timeStep in range(numTimeSteps):
            ...
            for index, layer in enumerate(self.layers):
                ...
                if nextLayer["dimension"] < layerDimension: # zero pad if feedback is small
                    # print(f"layer {layer},\n nextLayer {nextLayer}")
                    detectorIn += np.pad(nextLayer["mesh"].getFeedback() @ Diagonalize(np.pad(nextLayer["pAct"],
                                        (0, layerDimension-nextLayer["dimension"]))), (0, dimensionDifference))
                else:
                    detectorIn += nextLayer["mesh"].getFeedback() @ Diagonalize(nextLayer["pAct"])
                detectorOut = Detect(detectorIn)
                layer["pAct"] += deltaTime*(layer["rateCode"](detectorOut)[:layerDimension]-layer["pAct"])
