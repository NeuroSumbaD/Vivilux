...
if TYPE_CHECKING:
    ...
    from .paths import Path, LeabraPath
...
class Layer:
    '''A Layer is an abstraction representing a set of neurons. Layer objects
        are responsible for applying dynamic nonlinearities within the neural
        network. Should also define any functions necessary for updating the
        internal states of the Layer that are used for learning. The base class
        only defines type-checking features and the basic creation of
        connections to Path classes. Other functions used for updating internal
        states should be managed at the network level and correspond to the Net
        type (only Leabra for now).
    '''
    count = 0
    def __init__(self,
                 shape: tuple,
                 ...
                 signalParams = {},
                 **kwargs):
        self.inputType = signalIn
        self.dataOut = signalOut(shape, **signalParams)

    def ConnectPath(self, path: Path):
        self.VerifyPath(path)

    def VerifyPath(self, path: Path):
        if not issubclass(path.outputType, self.inputType):
            raise TypeError(f"{type(self)} does not accept output type {path.outputType} supported by path type {type(path)}.")

    def Get(self,):
        '''Returns the output signal generated by a call to the dataOut.Get()
        '''
        return self.dataOut.Get()

class LeabraLayer(Layer):
    '''Base class for a layer that implements the behavior of Layers using
    ...
    def __init__(self,
        ...
        self.excPaths: list[LeabraPath] = []
        self.inhPaths: list[LeabraPath] = []
    ...
    def Integrate(self):
        ...
        '''
        for path in self.excPaths:
            path.apply()
            self.GeRaw[:] += path.dataOut.Get()

        for path in self.inhPaths:
            path.apply()
            self.GiRaw[:] += path.dataOut.Get()
    ...
    def EndStep(self, time, debugData = None):
        ...
        self.dataOut.SetNorm(self.getActivity())
    ...
    def Learn(self, batchComplete=False, dwtLog = {}):
        ...
        for path in self.excPaths:
            if not path.trainable: continue
            path.Update(dwtLog=dwtLog)
    ...
    def ConnectPath(self, path: Path, excitatory = True):
        super().VerifyPath(path)
        self.VerifyPath(path)
        if excitatory:
            self.excPaths.append(path)
        else:
            self.inhPaths.append(path)
    ...
    def __str__(self) -> str:
        ...
        layStr += f"\n\tMeshes: " + "\n".join([str(mesh) for mesh in self.excPaths])
        layStr += f"\n\tActivity: \n\t\t{self.Act}"
        return layStr
