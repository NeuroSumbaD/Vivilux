'''Applies the calibrated optimum CRIREL parameters to the mesh and then takes
    steps in every direction of the parameter space to visualize the loss
    landscape.
...
with netlist:
    ...
    opt_mat = mzi.measureMatrix(np.zeros(mzi.shape))
    print(f"Initial matrix: \n{opt_mat.round(3)}")
    
    
    pins = mzi.psPins
    initial_params = mzi.getParams()[0].flatten().copy()**2
    voltage_ax = np.linspace(0, 25, 50)
    for (param1, param2) in combinations(pins, 2):
        param1_index, param2_index = (pins.index(param1), pins.index(param2))
    
        # Get initial optimum position to show as "X" on plot
        optimum_position = (initial_params[param1_index], initial_params[param2_index])
        # print(f"Optimum position for {param1},{param2} = {optimum_position}")
        
        # Sweep both params over full 5V range (linear sweep in V^2)
        loss_landscape = np.zeros((len(voltage_ax), len(voltage_ax)))
        for ((index1, volt1), (index2, volt2)) in tqdm(product(enumerate(voltage_ax), enumerate(voltage_ax)),
                                                       desc=f"Sweeping voltages for {param1}, {param2}",
                                                       total=len(voltage_ax)**2):
            new_params = initial_params.copy()
            new_params[param1_index] = volt1
            new_params[param2_index] = volt2
            
            mzi.setParams([new_params.reshape(-1,1)])
            mzi.setFromParams()
            # sleep(10e-3)
            matrix = mzi.measureMatrix(np.zeros(mzi.shape))
            
            loss = np.linalg.norm(target_matrix - matrix)
            loss_landscape[index1, index2] = loss
        
        # Plot heatmap of loss landscape
        fig, ax = plt.subplots()
        ax.scatter(*optimum_position, c="red", marker="x", label="optimum")
        im = ax.imshow(loss_landscape,
                       extent=(0,25,0,25),
                       origin="lower",
                       vmin = 0.0,
                       interpolation='bicubic',
                       )
        ax.set_xticks(voltage_ax)
        ax.set_xlabel(param1)
        ax.set_yticks(voltage_ax)
    ...
    print('Final blocking show to wait for all plots to be closed.')
    plt.show()