...
time.sleep(0.1)
ser.write(b"W C AAAAAAAA\n")  #Set PWM period to 16777216 clock cycles (approx 1 second at 16MHz)
resp = ser.readline()

time.sleep(0.1)
ser.write(b"W 10 AAAAAAAA\n")  #Set PWM period to 16777216 clock cycles (approx 1 second at 16MHz)
resp = ser.readline()

time.sleep(0.1)
ser.write(b"W 14 AAAAAAAA\n")  #Set PWM period to 16777216 clock cycles (approx 1 second at 16MHz)
resp = ser.readline()

time.sleep(0.1)
ser.write(b"W 18 AAAAAAAA\n")  #Set PWM period to 16777216 clock cycles (approx 1 second at 16MHz)
resp = ser.readline()
##END VIBRATION PERIOD SETTING
ser.write(b"W 8 F000000F\n")
print("Cofigured serial for laser control")

class RealtimeScope:
    """Non-blocking real-time oscilloscope with continuous acquisition."""
    
    def __init__(self, device_name="Dev1", channels=[0, 1, 2, 3, 4], target_fps=30, 
    ...
    def _acquisition_loop(self):
        ...
        try:
            ...
            last_toggle_time = time.time() - self.start_time
            current_state = False
                ...
                try:
                    ...
                    if (current_time - last_toggle_time >= 0.5):
                        ...
                        if (current_state == False):
                            ...
                            current_state = False
                            print("Toggling OFF")
                            ser.write(b"W 4 F0000000\n")
                            #ser.write(b"W 8 F0000000\n")
    ...
    def start(self):
        ...
        print("\nWaiting 10 seconds for lasers to turn on...")
        time.sleep(10)
        self.running = True
