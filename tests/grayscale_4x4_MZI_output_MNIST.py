'''In this script, we load the optimum MZI parameters to perform grayscaling
    ...
    3. Optional: Output filename for the processed grayscale images (default is 'grayscale_4x4_mzi_output.npz').
'''
...
output_file = path.abspath(sys.argv[3]) if len(sys.argv) > 3 else 'grayscale_4x4_mzi_output.npz'
...
for index, img in enumerate(mnist_images):
    ...
    gray_image = np.array([mzi.applyTo(np.pad(pixels, (0,1))) for pixels in img.reshape(-1, 3)])
    gray_image = gray_image[:,:1]  # Keep only the first channel of MZI output
